...
app.put("/api/teampage/:id", uploads.single("image"), async (req, res) => {
    try {
        ...
        const updatedMember = await Teampage.findByIdAndUpdate(id, updates, {
            new: true,
        });
        if (!updatedMember) {
            return res.status(404).json({ message: "Team member not found" });
...
app.put(
    ...
    async (req, res) => {
        ...
        try {
            ...
            const updatedclients = await Valueclient.findByIdAndUpdate(_id, updates, {
                new: true,
            });
            if (!updatedclients) {
            ...
            res
                .status(200)
                .json({
                    message: "Team clients updated successfully",
                    member: updatedclients,
                });
        } catch (error) {
            console.error("Error updating team member:", error);
            res.status(500).json({ message: "Internal Server Error" });
...
app.post("/api/lastworkadd", uploads.single("image_work"), async (req, res) => {
    ...
    try {
        ...
        const imagePath = req.file.path;
        const lastworkadd = new lastwork({
            name,
            description,
            category,
            work,
...
app.post(
    ...
    async (req, res) => {
        ...
        try {
            ...
            const clientrateadd = new clientrate({
                name,
                description,
                business,
                rate,
...
app.put(
    "/api/clientrate/:id",
    uploads.single("image_client_work"),
    async (req, res) => {
        try {
            const { id } = req.params;
            const updates = { ...req.body };
            ...
            const updatedClientRate = await clientrate.findByIdAndUpdate(
                id,
                updates,
                { new: true }
            );
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (!savedserviceadd) {
                return res
                    .status(500)
                    .json({ message: "Failed to save service to the database." });
            }
            ...
            res
                .status(500)
                .json({ message: "Failed to add service. Please try again later." });
        }
    }
);
...
app.get("/api/service", async (req, res) => {
    try {
        ...
        res
            .status(500)
            .json({ message: "Failed to fetch services. Please try again later." });
    }
});
...
app.put(
    ...
    async (req, res) => {
        ...
        try {
            ...
            res
                .status(500)
                .json({ message: "Failed to update service. Please try again later." });
        }
    }
);
...
app.delete("/api/service/:id", async (req, res) => {
    try {
        ...
        res
            .status(500)
            .json({ message: "Failed to delete service. Please try again later." });
    }
});
...
app.post(
    "/api/blogpage",
    uploads.single("image_blog_work"),
    async (req, res) => {
        console.log(req.body);
        try {
            const { name } = req.body;
...
app.post("/api/contactdetails", async (req, res) => {
    ...
    try {
        ...
        const contactdetailsadd = new contactdetails({
            address,
            phoneno,
            avaliablity,
            email,
...
app.post("/api/setstatic", async (req, res) => {
    try {
        const { successfulproject, joiningcomparies, registeredcustomers } =
            req.body;
        const setstaticadd = new SetStatic({
            successfulproject,
            joiningcomparies,
            registeredcustomers,
        });
...
app.put("/api/setstatic", async (req, res) => {
    try {
        const { id, successfulproject, joiningcomparies, registeredcustomers } =
            req.body;
